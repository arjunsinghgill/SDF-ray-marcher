...
const HTTPS_ROUTE_home_files = {
  html : fs.readFileSync( path.resolve(__dirname,'./src.sdf_ray_marcher.html') ),
  js   : fs.readFileSync( path.resolve(__dirname,'./src.sdf_ray_marcher.js') ),
  css  : fs.readFileSync( path.resolve(__dirname,'./src.sdf_ray_marcher.css') ),
};
...
const HTTPS_TLS_SERVER = https.createServer( HTTPS_TLS_CERTS, async (req,res) => {
  ...
  try {
    // parse the url
    let TMP_url_parsed = new URL( req.url, "https://localhost:8085/" );
    // store the pathname
...
console.log('HTTPS server running at port : 8085');
